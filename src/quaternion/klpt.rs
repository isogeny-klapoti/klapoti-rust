...
use crate::{
    ...
    quaternion::{
        ...
        quaternion_order::standard_extremal_order,
    },
...
pub fn keygen_klpt(ideal: QuaternionIdeal, qa: QuatAlg) -> Result<QuatAlgEl> {
    ...
    while cnt < KLPT_KEYGEN_NUM_GAMMA_TRIAL {
        ...
        let order = standard_extremal_order().order;
...
pub fn klpt(
    ...
    let (gamma, _) = match represent_integer(n_gamma.clone(), qa.clone(), order.clone()) {
...
mod tests {
    ...
    fn solve_combi_eichler_test() {
        ...
        let order = standard_extremal_order().order;
        let (gamma1, _) = represent_integer(temp, qa.clone(), order.clone()).unwrap();

        temp = 2.big().pow(exp as u32 * 3);
        let (shift_gamma, temp) = represent_integer(temp, qa.clone(), order.clone()).unwrap();

        let (delta, _) = represent_integer(temp, qa.clone(), order.clone()).unwrap();
    ...
    fn keygen_klpt_test() {
        ...
        tmp = tmp * n_tau.clone();
        let order = standard_extremal_order().order;
        let (gamma_tau, _) = represent_integer(tmp, qa.clone(), order.clone()).unwrap();
